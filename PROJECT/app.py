...
def edit(pid):
    ...
    if request.method=="POST":
        ...
        sect=request.form.get('sect')
        number=request.form.get('number')
        query = db.engine.execute(f"INSERT INTO `trigr` (`pid`, `email`, `name`, `action`, `timestamp`) VALUES ('{pid}','{email}','{name}','SLOT UPDATED','{datetime.now()}')")
        query.close()
        db.engine.execute(f"UPDATE `mahilas` SET `email` = '{email}', `name` = '{name}', `scheme` = '{scheme}', `amount` = '{amount}', `business` = '{business}', `time` = '{time}', `date` = '{date}', `sect` = '{sect}', `number` = '{number}' WHERE `mahilas`.`pid` = {pid}")
...
@login_required
def delete(pid):
    user = Mahilas.query.filter_by(pid=pid).first()
    query = db.engine.execute(f"INSERT INTO `trigr` (`pid`, `email`, `name`, `action`, `timestamp`) VALUES ('{pid}','{user.email}','{user.name}','SLOT DELETED','{datetime.now()}')")
    query.close()
